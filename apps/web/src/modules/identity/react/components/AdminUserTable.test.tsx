...
describe('AdminUserTable', () => {
  it('says so when no account answers the filters', () => {
    render(<AdminUserTable users={[]} action={action} />)

    expect(screen.getByText(/aucun compte ne répond à ces filtres/i)).toBeVisible()
  })

  it('names the account and its address', () => {
    render(<AdminUserTable users={[adminUserFixture({ displayName: 'Inès Ferrand' })]} action={action} />)
  ...
  it('offers to name a plain member administrator', () => {
    render(<AdminUserTable users={[adminUserFixture()]} action={action} />)
  ...
  it('offers to take the administration back from an admin', () => {
    render(<AdminUserTable users={[adminUserFixture({ platformRole: 'PLATFORM_ADMIN' })]} action={action} />)
  ...
  it('offers to reactivate a suspended account rather than to suspend it again', () => {
    render(<AdminUserTable users={[adminUserFixture({ status: 'SUSPENDED' })]} action={action} />)
  ...
  it('says out loud that the list is capped', () => {
    ...
    render(<AdminUserTable users={users} action={action} />)
