...
import { BookingUnknownError } from '../../domain/errors/booking.errors.ts'
...
export class GetBookingDetailUsecase {
  ...
  async execute(user: AuthUser, bookingId: string): Promise<BookingDetail> {
    ...
    return toBookingDetail(booking, machine, this.clock.now())
  }
}
