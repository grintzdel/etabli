...
import { BookingNotMarkableAsNoShowError, BookingUnknownError } from '../../domain/errors/booking.errors.ts'
...
export class MarkNoShowUsecase {
  ...
  async execute(user: AuthUser, bookingId: string): Promise<AtelierBooking> {
    ...
    if (!booking.isNoShowMarkable(now)) throw new BookingNotMarkableAsNoShowError(bookingId, booking.status)

    const marked = await this.bookingRepository.markNoShow(bookingId, now)
    if (marked === null) throw new BookingUnknownError(bookingId)

    const names = await this.userRepository.namesOf([marked.userId])
